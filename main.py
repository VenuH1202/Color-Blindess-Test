...
class Quiz:
    ...
    def __init__(self):
      ...
      # assigns ques to the display_question function to update later.
        ...
        self.correct = 0
        mb.showinfo("Instructions","This is a colour Blindness test. You need to answer the question after properly focussing on the image")
    ...
    def display_result(self):
        ...
        df = df.dropna()
        df["Out"] = df["Result"].apply(lambda x: 0 if x==0 else 1)
        X = np.array(df["Score"]).reshape(-1,1)
        Y = np.array(df["Out"]).reshape(-1,1)
        ...
        res = [model.predict(np.array([score]).reshape(-1,1)),
            knn.predict(np.array([score]).reshape(-1,1)),
            clf_gini.predict(np.array([score]).reshape(-1,1)),
            nb.predict(np.array([score]).reshape(-1,1))]
        
        # Shows a message box to display the result
        if res.count(1)>=res.count(0):
            mb.showinfo("Result","You don't have Color blindness")
        else:
            mb.showinfo("Result","You have Color blindness")
    ...
    def display_question(self):
        ...
        img = Image.open(str(question[self.q_no]))
        ph = ImageTk.PhotoImage(img)

        # setting the Question properties
        q_no = Label(gui,image=ph,height=300,width=250)
        q_no.image = ph

        # placing the option on the screen
        q_no.place(x=350, y=50)

    # This method is used to Display Title
    def display_title(self):

        # The title to be shown
        title = Label(gui, text="Color Blindness Testing App",
...
gui.title("Color Blindness Test App")
